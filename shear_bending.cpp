...
///////////////////////////////////////////////////////////////////////////////////distributed force below//////////////////////////////////////////////////////////////////////////////
			...
			if (force_type == 'l') {							//distributed load setting//
				...
				if (force_direction_distributed == 'u') {		//upward force for distributed load
					...
					for (int i = 0; i < store_load_force_magnitude_per_meter.size(); i++) {				//for each load force available
						for (double j = store_load_force_location_begin[i]; j <= store_load_force_location_end[i]; j += 0.2) {
							double shear_load = store_load_force_magnitude_per_meter[i] * (abs(store_load_force_location_begin[i] - j)); // computes the end point
							cout << shear_load << " ," << j << endl;
	...
	void setSupports() {
		double support_location; char escape; int InitSupport = 0;			//initiate the supprot value to be zero.
		while (1) {
...
public:
	void Compute_support_reaction() {
		...
		if (store_load_force_location_begin.size() > 0) {												//checks for load force presence.
			for (int i = 0; i < store_load_force_magnitude_per_meter.size(); i++) {						//for each looad force element
				double storeCentroid = 0.5*(store_load_force_location_begin[i] - store_load_force_location_end[i]);		//store the centroid value for each load
	...
	void computeShearForce(){
		...
		for (int force_plot = 0; force_plot < shear_force.size() - 1; force_plot++) {		//goes through each loaction, except the last one (last element)
				for (double i = vectPair[force_plot].first; i <= vectPair[force_plot + 1].first; i += 0.2) {	//further split the locations into increments of 0.2
