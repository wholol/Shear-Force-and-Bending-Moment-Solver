...
protected:										//inheritance used, hence protected
	...
	vector <double> distributed_w_value;				//MAGNITUDE of load force. in N/m
...
public:
		...
		while (1) {
			...
			if (force_type == 'p') {													// point load.
				...
				if (force_direction == 'd') {												//downward force
					...
					store_point_force_magnitude.push_back(force_value*-1);			//store point force magnitude
	...
	void setSupports() {
		double support_location; int InitSupport = 9999;			//initiate the supprot value to be zero.
		while (1) {
	...
	void Compute_support_reaction() {
		...
		if (point_force_location.size() > 0) {					//checks for point force presence.
			for (int i = 0; i < point_force_location.size(); i++) {		//loop through all point forces
				double pointForceMoments = store_point_force_magnitude[i] * (point_force_location[i] - store_support_location[0]);	//compute the moments for each point
		...
		if (store_load_force_location_begin.size() > 0) {												//checks for load force presence.
			for (int i = 0; i < distributed_w_value.size(); i++) {						//for each looad force element
				...
				double storeLoadForce = distributed_w_value[i] * (abs(store_load_force_location_begin[i] - store_load_force_location_end[i]));	//force*distance for force value.location not calculated.												
		...
		double storeSupport = -(sum_moments_point_force + sum_moments_load_force) / (abs(store_support_location[1] - store_support_location[0])); //compute first support value. Fsupport = Moment/distance
		...
		for (int i = 0; i < store_force.size(); i++) {																
			if (store_force[i] == 9999 && store_location[i] == store_support_location[1]) {			//detects if a store force element is 9999, since the supports were initialized to be 9999
	...
	void computeShearForce(){					//function to compute shear force.
		for (int i = 0; i < distributed_w_value.size(); i++) {						//for each load force available
			for (float j = store_load_force_location_begin[i]; j <= store_load_force_location_end[i]; j += 0.2) {	//further split the location into 0.2
				float shear_load = distributed_w_value[i] * (abs(store_load_force_location_begin[i] - j));			//computes the end point
				...
				if (vectPair[k].second == -0 || vectPair[k].second == 0) {					//if the load force detected is zero
					...
					for (int j = 0; j < distributed_w_value.size(); j++) {			//goes through each possible load forces
						for (float z = store_load_force_location_begin[j]; z <= store_load_force_location_end[j]; z += 0.2) {	//loops through load force region
							float shear_load = vectPair[k].second + distributed_w_value[j] * (abs(store_load_force_location_begin[j] - z)); //linear shear subtraction
